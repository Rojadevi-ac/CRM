...
export default function GlobalSearchModal({ isOpen, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-20 p-4">
      ...
      <div className="relative w-full max-w-2xl clay-card overflow-hidden z-10 animate-scaleIn">
        {/* Search Input Bar */}
        <div className="flex items-center p-3.5 border-b border-slate-100 dark:border-slate-800 gap-3">
          <Search className="w-5 h-5 text-brand-500 ml-1 shrink-0" />
          <input
            ref={inputRef}
            type="text"
            value={query}
            ...
            className="flex-1 px-3 py-2 clay-inset text-slate-900 dark:text-slate-100 placeholder-slate-400 text-sm font-semibold focus:outline-none focus:ring-1 focus:ring-brand-500"
          />
          {query && (
            <button onClick={() => setQuery('')} className="p-1.5 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200">
              <X className="w-4 h-4" />
            </button>
          )}
        </div>
