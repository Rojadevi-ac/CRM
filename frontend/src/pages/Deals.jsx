      ...
      <div className="p-4 sm:p-5 clay-card flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[220px]">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 clay-inset text-xs text-slate-800 dark:text-slate-100 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-500/30"
          />
        </div>
        ...
        <select
          ...
          className="px-4 py-2.5 clay-pill text-xs font-bold text-slate-700 dark:text-slate-200 focus:outline-none"
        >
        ...
        {(search || ownerFilter) && (
          <button
            ...
            className="px-3.5 py-2 text-xs font-extrabold text-slate-600 dark:text-slate-300 clay-pill flex items-center gap-1.5 active:scale-95"
          >
            <RotateCcw className="w-3.5 h-3.5 text-brand-600" /> Reset
          </button>
        )}
      </div>

      {/* Kanban Pipeline View (Fixed Overlap & Pure Claymorphic Styling) */}
      {viewMode === 'kanban' ? (
              ...
              >
                ...
                <div className="flex items-center justify-between pb-3 mb-3 border-b border-slate-200/60 dark:border-slate-800">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-extrabold text-slate-400">({stageDeals.length})</span>
                  </div>
                ...
                {/* Cards Container with smooth scrolling and isolated spacing */}
                <div className="flex-1 overflow-y-auto space-y-3.5 p-1 pr-1.5">
                  {stageDeals.length === 0 ? (
                    <div className="p-8 text-center text-xs font-bold text-slate-400 clay-inset">No deals in this stage</div>
                  ) : (
                    stageDeals.map((deal) => {
                      ...
                      return (
                        <div
                          key={deal.id}
                          className={`p-4 clay-card transition-all space-y-3 select-none ${
                            isLost
                        ...
                        >
                          ...
                          <div className="flex items-start justify-between gap-2">
                            <span className="text-xs font-extrabold text-slate-900 dark:text-slate-100 line-clamp-1 flex-1">
                              {deal.deal_name}
                            </span>
                            <div className="flex items-center gap-1.5 shrink-0">
                              {isLost ? (
                                <span className="text-[10px] px-2 py-0.5 rounded-full bg-rose-100 dark:bg-rose-950 font-extrabold text-rose-700 dark:text-rose-300 border border-rose-300 dark:border-rose-800">
                                  Lost
                                </span>
                              ) : isWon ? (
                                <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-950 font-extrabold text-emerald-700 dark:text-emerald-300 border border-emerald-300 dark:border-emerald-800">
                                  Won
                                </span>
                              ) : (
                                <span className="text-[10px] px-2 py-0.5 rounded-full clay-pill font-extrabold text-slate-600 dark:text-slate-300">
                                  {deal.probability}%
                                </span>
                              )}
                              {canWrite && (
                                <div className="flex items-center gap-1">
                                  <button
                                    ...
                                    title="Edit Deal"
                                    className="p-1 text-slate-400 hover:text-brand-600 rounded-lg clay-pill transition-colors active:scale-90"
                                  >
                                  ...
                                  <button
                                    ...
                                    title="Delete Deal"
                                    className="p-1 text-slate-400 hover:text-rose-600 rounded-lg clay-pill transition-colors active:scale-90"
                                  >
                          ...
                          <div className="text-[11px] text-slate-500 dark:text-slate-400 truncate flex items-center gap-1.5 font-medium">
                            ...
                            <span className="truncate">{deal.customer_name || deal.lead_name || 'Prospect'}</span>
                          </div>
                          ...
                          <div className="flex items-center gap-1 text-[10px] text-slate-500 dark:text-slate-400 clay-inset px-2.5 py-1 font-medium">
                            <Clock className="w-3 h-3 text-slate-400 shrink-0" />
                            <span className="truncate">
                              {formatDateTime12Hour(deal.updated_at || deal.created_at)}
                            </span>
                          </div>
                          ...
                          {canWrite && (
                            <div className="pt-1 space-y-1.5">
                              {!isLost && !isWon ? (
                                <div className="grid grid-cols-2 gap-2">
                                  <button
                                    ...
                                    className="py-1.5 px-2 text-[10px] font-extrabold text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/70 hover:bg-emerald-100 dark:hover:bg-emerald-900 border border-emerald-200 dark:border-emerald-800 rounded-xl flex items-center justify-center gap-1 transition-all active:scale-95 shadow-soft-xs"
                                  >
                                  ...
                                  <button
                                    ...
                                    className="py-1.5 px-2 text-[10px] font-extrabold text-rose-700 dark:text-rose-300 bg-rose-50 dark:bg-rose-950/70 hover:bg-rose-100 dark:hover:bg-rose-900 border border-rose-200 dark:border-rose-800 rounded-xl flex items-center justify-center gap-1 transition-all active:scale-95 shadow-soft-xs"
                                  >
                                ...
                                <button
                                  ...
                                  className="w-full py-1.5 px-2 text-[10px] font-bold text-slate-700 dark:text-slate-300 clay-btn-secondary flex items-center justify-center gap-1.5"
                                >
                                  <RotateCcw className="w-3.5 h-3.5 text-brand-600" /> Reopen Deal
                                </button>
                              ) : (
                                <button
                                  type="button"
                                  ...
                                  className="w-full py-1.5 px-2 text-[10px] font-bold text-slate-700 dark:text-slate-300 clay-btn-secondary flex items-center justify-center gap-1.5"
                                >
                                  <RotateCcw className="w-3.5 h-3.5 text-indigo-600" /> Reopen to Negotiation
                                </button>
                              )}

                              {/* Stage Switch Dropdown */}
                              <select
                                value={deal.stage}
                              ...
                              >
                                {DEAL_STAGES.map((st) => (
                                  <option key={st} value={st} className="bg-white dark:bg-[#131b2c]">{st}</option>
                                ))}
                              </select>
                            </div>
                          )}
        ...
        <DataTable
          ...
          emptyDescription="Start tracking deals in your pipeline."
