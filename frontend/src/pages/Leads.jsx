    ...
    <div className="space-y-5 pb-12">
      ...
      <div className="p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-xs flex flex-wrap items-center gap-3">
        ...
        <select
          ...
          className="px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-xs text-slate-700 dark:text-slate-300 focus:outline-none focus:border-brand-500"
        >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-7 gap-4 overflow-x-auto pb-4">
          {LEAD_STATUSES.map((status) => {
            ...
            return (
              <div
                key={status}
                className="bg-slate-100/70 dark:bg-slate-850/60 rounded-xl p-3 border border-slate-200 dark:border-slate-800 min-w-[240px] flex flex-col h-[75vh]"
              >
                {/* Column Header */}
                ...
                <div className="flex-1 overflow-y-auto space-y-2.5 pr-1">
                  {statusLeads.length === 0 ? (
                    <div className="p-4 text-center text-xs text-slate-400">No leads in {status}</div>
                  ) : (
                    statusLeads.map((lead) => (
                      <div
                        key={lead.id}
                        className="p-3 bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 hover:shadow-md transition-all space-y-2 select-none"
                      >
                        <div className="flex items-start justify-between gap-1">
                          <Link
                            to={`/leads/${lead.id}`}
                            className="text-xs font-bold text-slate-900 dark:text-slate-100 hover:text-brand-600 line-clamp-1"
                          >
                            {lead.name}
                          </Link>
                        ...
                        {canWrite && (
                          <div className="pt-1.5 flex items-center justify-between">
                            <select
                              ...
                              className="text-[10px] py-0.5 px-1 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded text-slate-600 dark:text-slate-300 w-full"
                            >
                              {LEAD_STATUSES.map((st) => (
